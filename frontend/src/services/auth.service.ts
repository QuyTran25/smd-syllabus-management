...
export const authService = {
  ...
  getCurrentUser: async (token: string): Promise<User> => {
    ...
    try {
      ...
      // Map backend user info to frontend User type
      const rawRole = userInfo.roles && userInfo.roles.length > 0 ? userInfo.roles[0] : 'Lecturer';
      console.log('🔍 [authService.getCurrentUser] Backend returned roles:', userInfo.roles, 'First role:', rawRole);
      ...
      console.error('❌ [authService.getCurrentUser] GET /api/auth/me FAILED:', error);
      throw error;
    }
  },
