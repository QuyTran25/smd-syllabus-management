...
UPDATE core_service.subjects s SET
    subject_type = CASE 
        WHEN s.code IN ('101001', '101002', '101003', '102001', '102002', '103001') THEN 'REQUIRED'::core_service.subject_type
        ELSE CASE WHEN random() < 0.7 THEN 'REQUIRED'::core_service.subject_type ELSE 'ELECTIVE'::core_service.subject_type END
    END,
    component = CASE 
        WHEN s.code LIKE '10%' THEN 'BOTH'::core_service.subject_component
        WHEN s.code LIKE '11%' THEN 'THEORY'::core_service.subject_component
        WHEN s.code LIKE '12%' THEN 'BOTH'::core_service.subject_component
        ELSE 'BOTH'::core_service.subject_component
    END,
    default_theory_hours = s.default_credits * 15,
    default_practice_hours = CASE 
        WHEN s.code LIKE '11%' THEN 0 
...
-- First ensure PLOs exist (FIXED: Case sensitive Enum values)
INSERT INTO core_service.plos (id, curriculum_id, code, description, category, created_at, updated_at)
SELECT gen_random_uuid(), c.id, 'PLO1', 'Có khả năng áp dụng kiến thức toán học, khoa học và kỹ thuật', 'Knowledge', NOW(), NOW() -- Sửa KNOWLEDGE -> Knowledge
FROM core_service.curriculums c
WHERE NOT EXISTS (SELECT 1 FROM core_service.plos WHERE code = 'PLO1' AND curriculum_id = c.id);

INSERT INTO core_service.plos (id, curriculum_id, code, description, category, created_at, updated_at)
SELECT gen_random_uuid(), c.id, 'PLO2', 'Có khả năng phân tích và giải quyết vấn đề trong lĩnh vực CNTT', 'Skills', NOW(), NOW() -- Sửa SKILLS -> Skills
FROM core_service.curriculums c
WHERE NOT EXISTS (SELECT 1 FROM core_service.plos WHERE code = 'PLO2' AND curriculum_id = c.id);

INSERT INTO core_service.plos (id, curriculum_id, code, description, category, created_at, updated_at)
SELECT gen_random_uuid(), c.id, 'PLO3', 'Có khả năng thiết kế hệ thống hoặc quy trình đáp ứng yêu cầu', 'Skills', NOW(), NOW() -- Sửa SKILLS -> Skills
FROM core_service.curriculums c
WHERE NOT EXISTS (SELECT 1 FROM core_service.plos WHERE code = 'PLO3' AND curriculum_id = c.id);

INSERT INTO core_service.plos (id, curriculum_id, code, description, category, created_at, updated_at)
SELECT gen_random_uuid(), c.id, 'PLO4', 'Có khả năng làm việc nhóm hiệu quả', 'Attitude', NOW(), NOW() -- Sửa ATTITUDES -> Attitude (Chú ý số ít/nhiều theo V8, thường là Attitude hoặc Attitudes, check kỹ file V8)
FROM core_service.curriculums c
WHERE NOT EXISTS (SELECT 1 FROM core_service.plos WHERE code = 'PLO4' AND curriculum_id = c.id);

INSERT INTO core_service.plos (id, curriculum_id, code, description, category, created_at, updated_at)
SELECT gen_random_uuid(), c.id, 'PLO5', 'Có khả năng giao tiếp hiệu quả bằng văn bản và thuyết trình', 'Attitude', NOW(), NOW() -- Sửa ATTITUDES -> Attitude
FROM core_service.curriculums c
WHERE NOT EXISTS (SELECT 1 FROM core_service.plos WHERE code = 'PLO5' AND curriculum_id = c.id);
