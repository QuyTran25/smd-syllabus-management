...
const SyllabusReviewPage: React.FC = () => {
  ...
  // Fetch syllabi with PENDING_HOD status
  const { data, isLoading } = useQuery({
    queryKey: ['syllabi-pending-hod', pagination],
    queryFn: () =>
  ...
  const getStatusTag = (status: SyllabusStatus) => {
    const statusConfig: Record<string, { color: string; text: string }> = {
      [SyllabusStatus.PENDING_HOD]: { color: 'orange', text: 'Chờ Trưởng BM duyệt' },
      [SyllabusStatus.PENDING_HOD_REVISION]: { color: 'gold', text: 'Chờ duyệt (sửa lại)' },
      ...
      <Card>
        <Space direction="vertical" size="large" style={{ width: '100%' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <Text type="secondary">
              Danh sách đề cương chờ Trưởng bộ môn duyệt
            </Text>
          </div>

          {/* Search */}
          <Input
            placeholder="Tìm kiếm theo mã môn, tên môn học hoặc giảng viên..."
