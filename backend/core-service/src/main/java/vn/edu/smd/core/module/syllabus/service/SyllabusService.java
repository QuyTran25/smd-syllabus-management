...
public class SyllabusService {
    ...
    public Page<SyllabusResponse> getAllSyllabi(Pageable pageable, List<String> statusStrings) {
        ...
        if (statusStrings != null && !statusStrings.isEmpty()) {
            ...
            // Use eager-loading query to fetch createdBy for access control checks
            List<SyllabusVersion> allResults = syllabusVersionRepository.findByStatusInAndIsDeletedFalseWithCreatedBy(statuses);
            
            // Apply ownership-based access control for LECTURER role
            List<SyllabusVersion> filteredResults = applyAccessControl(allResults, currentUser);
            
            List<SyllabusResponse> responses = filteredResults.stream()
    ...
    /**
     * Apply access control based on user role and ownership
     * LECTURER: Can see own syllabi at all statuses, but others' syllabi only if PUBLISHED or ARCHIVED
     * Other roles: Can see all statuses (role-based filtering already applied)
     */
    private List<SyllabusVersion> applyAccessControl(List<SyllabusVersion> syllabi, User currentUser) {
        ...
        // Only LECTURER needs ownership-based filtering
        if ("LECTURER".equals(primaryRole)) {
            UUID currentUserId = currentUser.getId();
            return syllabi.stream()
                    .filter(syllabus -> {
                        // Show if owned by current user (regardless of status)
                        if (syllabus.getCreatedBy() != null && 
                            syllabus.getCreatedBy().getId() != null &&
                            syllabus.getCreatedBy().getId().equals(currentUserId)) {
                            return true;
                        }
                        // Show if not owned but status is PUBLISHED or ARCHIVED
                        SyllabusStatus status = syllabus.getStatus();
                        return status != null && (
                            SyllabusStatus.PUBLISHED == status || 
                            SyllabusStatus.ARCHIVED == status
                        );
                    })
                    .collect(Collectors.toList());
        }
        
        // Other roles: no additional filtering needed
        return syllabi;
    }
    ...
    public SyllabusResponse createSyllabus(SyllabusRequest request) {
        ...
        SyllabusVersion syllabus = SyllabusVersion.builder()
                ...
                .componentType(ComponentType.MAJOR)
                .build();
    ...
    public Map<String, Integer> getStatistics() {
        ...
        for (SyllabusStatus status : SyllabusStatus.values()) {
            // ⭐ QUAN TRỌNG: Hàm countByStatusAndIsDeletedFalse CẦN ĐƯỢC THÊM VÀO REPOSITORY (Xem Phần 2)
            long count = syllabusVersionRepository.countByStatusAndIsDeletedFalse(status);
            stats.put(status.name(), (int) count);
        }
        return stats;
    }
    ...
    private SyllabusResponse mapToResponse(SyllabusVersion syllabus) {
        ...
        response.setIsDeleted(syllabus.getIsDeleted());
        ...
        response.setAssessmentMethods(assessments.stream().map(as -> {
            ...
            // Logic mapping tên sang phương thức/hình thức (giữ nguyên logic của bạn)
            String name = as.getName() != null ? as.getName().toLowerCase() : "";
